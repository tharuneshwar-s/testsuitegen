...
# Operation: add_comment_comments_add_post
...
def test_data_setup(api_client):
    ...
    # Create comment for HAPPY_PATH
    comment_payload = {"post_id": 1, "user_id": 1, "text": "This is a sample comment."}
    response = api_client.post(f"{BASE_URL}/comments", json=comment_payload)
    if response.status_code == 201:
        created_resources.append(
            {
                "type": "comment",
                "id": response.json()["id"],
                "endpoint": f"{BASE_URL}/comments/{response.json()['id']}",
            }
        )
...
class TestAdd_comment_comments_add_post:
    """Test suite for POST /comments/add endpoint."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                "HAPPY_PATH",
                {"post_id": 1, "user_id": 1, "text": "This is a sample comment."},
                {},
                201,
                id="HAPPY_PATH",
            ),
            pytest.param(
                ...
                {"user_id": 1, "text": "This is a sample comment."},
                {},
                400,
            ...
            pytest.param(
                "REQUIRED_FIELD_MISSING",
                {"post_id": 1, "text": "This is a sample comment."},
                {},
                400,
                id="REQUIRED_FIELD_MISSING",
            ),
            ...
            pytest.param(
                "NULL_NOT_ALLOWED",
                {"post_id": None, "user_id": 1, "text": "This is a sample comment."},
                {},
                400,
                id="NULL_NOT_ALLOWED",
            ),
            ...
            pytest.param(
                "NULL_NOT_ALLOWED",
                {"post_id": 1, "user_id": None, "text": "This is a sample comment."},
                {},
                400,
                id="NULL_NOT_ALLOWED",
            ),
            ...
            pytest.param(
                "SQL_INJECTION",
                {"post_id": 1, "user_id": 1, "text": "' OR '1'='1"},
                {},
                422,
                id="SQL_INJECTION",
            ),
            pytest.param(
                "XSS_INJECTION",
                {"post_id": 1, "user_id": 1, "text": "<script>alert(1)</script>"},
                {},
                422,
                id="XSS_INJECTION",
            ),
    ...
    def test_add_comment_comments_add_post_contract(
        self, api_client, intent, payload, path_params, expected_status
    ):
        """
        Validates POST /comments/add against the OpenAPI contract.
        Intent: {intent}
