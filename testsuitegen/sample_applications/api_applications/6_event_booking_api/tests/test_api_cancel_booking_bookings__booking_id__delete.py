...
# Operation: cancel_booking_bookings__booking_id__delete
...
def test_data_setup(api_client):
    ...
    created_booking_payload = {
        "event_id": "EVT-1234",
        "attendee_name": "Alice Smith",
        "attendee_email": "alicesmith@example.com",
        "quantity": 1,
    }
    ...
    for _key in created_booking_payload:
        if isinstance(created_booking_payload[_key], str):
            if "email" in _key.lower():
                parts = created_booking_payload[_key].split("@")
                if len(parts) == 2:
                    created_booking_payload[_key] = (
                        f"{parts[0]}_{_unique_suffix}@{parts[1]}"
                    )
            elif "name" in _key.lower():
                created_booking_payload[_key] = (
                    f"{created_booking_payload[_key]}_{_unique_suffix}"
                )

    created_booking_response = api_client.post(
        f"{BASE_URL}/bookings", json=created_booking_payload
    )

    if created_booking_response.status_code in (200, 201):
        created_booking_data = created_booking_response.json()
        created_booking_id = created_booking_data.get("id")
        if created_booking_id:
            created_resources.append(
                {
                    "type": "booking",
                    "id": created_booking_id,
                    "data": created_booking_data,
                    "endpoint": f"{BASE_URL}/bookings/{created_booking_id}",
                }
            )
            # Map placeholder to created ID
            placeholders["USE_CREATED_RESOURCE"] = created_booking_id
            placeholders["USE_CREATED_RESOURCE_BOOKING"] = created_booking_id
    else:
        # Creation failed - tests needing this resource will be skipped
        print(
            f"Warning: Could not create booking: {created_booking_response.status_code} - {created_booking_response.text}"
        )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "HAPPY_PATH",
            {},
            {"booking_id": "USE_CREATED_RESOURCE"},
            200,
            id="HAPPY_PATH",
        ),
        pytest.param(
            "RESOURCE_NOT_FOUND",
            {},
            {"booking_id": "nonexistent_resource"},
            404,
            id="RESOURCE_NOT_FOUND",
        ),
    ],
)
def test_cancel_booking_bookings__booking_id__delete_contract(
    api_client, test_data_setup, intent, payload, path_params, expected_status
):
    """
    Validates DELETE /bookings/{booking_id} against the OpenAPI contract.
    Intent: {intent}
