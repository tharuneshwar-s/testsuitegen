...
def test_data_setup(api_client):
    ...
    # Create a single active user for the happy path
    res1 = api_client.post(
        f"{BASE_URL}/users", json={"name": "Active User", "status": "active"}
    )
...
class TestUpdate_limit_wallets_limit_post:
    """Test suite for POST /wallets/limit endpoint."""
