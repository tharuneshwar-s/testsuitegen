...
# Operation: get_transfer_transfers__transfer_id__get
...
def test_data_setup(api_client):
    ...
    created_transfer_payload = {}

    # Make unique to avoid conflicts on repeated runs
    import uuid as _uuid

    _unique_suffix = _uuid.uuid4().hex[:8]
    for _key in created_transfer_payload:
        if isinstance(created_transfer_payload[_key], str):
            if "email" in _key.lower():
                parts = created_transfer_payload[_key].split("@")
                if len(parts) == 2:
                    created_transfer_payload[_key] = (
                        f"{parts[0]}_{_unique_suffix}@{parts[1]}"
                    )
            elif "name" in _key.lower():
                created_transfer_payload[_key] = (
                    f"{created_transfer_payload[_key]}_{_unique_suffix}"
                )

    created_transfer_response = api_client.post(
        f"{BASE_URL}/transfers", json=created_transfer_payload
    )

    if created_transfer_response.status_code in (200, 201):
        created_transfer_data = created_transfer_response.json()
        created_transfer_id = created_transfer_data.get("id")
        if created_transfer_id:
            created_resources.append(
                {
                    "type": "transfer",
                    "id": created_transfer_id,
                    "data": created_transfer_data,
                    "endpoint": f"{BASE_URL}/transfers/{created_transfer_id}",
                }
            )
            # Map placeholder to created ID
            placeholders["USE_CREATED_RESOURCE"] = created_transfer_id
            placeholders["USE_CREATED_RESOURCE_TRANSFER"] = created_transfer_id
    else:
        # Creation failed - tests needing this resource will be skipped
        print(
            f"Warning: Could not create transfer: {created_transfer_response.status_code} - {created_transfer_response.text}"
        )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "HAPPY_PATH",
            {},
            {"transfer_id": "USE_CREATED_RESOURCE"},
            200,
            id="HAPPY_PATH",
        ),
        pytest.param(
            "RESOURCE_NOT_FOUND",
            {},
            {"transfer_id": "nonexistent_resource"},
            404,
            id="RESOURCE_NOT_FOUND",
        ),
    ],
)
def test_get_transfer_transfers__transfer_id__get_contract(
    api_client, test_data_setup, intent, payload, path_params, expected_status
):
    """
    Validates GET /transfers/{transfer_id} against the OpenAPI contract.
    Intent: {intent}
